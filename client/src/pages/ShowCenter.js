...
export default function ShowCenter() {
  ...
  const getCenters = async () => {
    try {
      ...
      const res = await axios.get(
        "http://localhost:8080/api/v1/admin/showCenter",
        {
          headers,
        }
      );
  ...
  const updateCenters = (id) => {
    ...
    //     `http://localhost:8080/api/v1/admin/getCenter/${id}`,
    //     {
    //       headers: {
  ...
  const handleUpdate = async (values) => {
    try {
      const res = await axios.put(
        "http://localhost:8080/api/v1/admin/putCenter",
        { ...values, _id: updUser },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const deleteCenters = async (id) => {
    ...
    try {
      const res = await axios.delete(
        `http://localhost:8080/api/v1/admin/deleteCenter/${id}`
      );
    } catch (err) {
      console.log(err);
    }
        ...
        <Form onFinish={handleUpdate}>
          ...
          <Col xs={24} md={24} lg={8}>
            <Form.Item label="dose1" name="dose1">
              <Input type="number" placeholder="dose 1" />
            </Form.Item>
          </Col>
          <Col xs={24} md={24} lg={8}>
            <Form.Item label="dose2" name="dose2">
              <Input type="number" placeholder="dose 1" />
            </Form.Item>
          </Col>{" "}
          <Col xs={24} md={24} lg={8}>
